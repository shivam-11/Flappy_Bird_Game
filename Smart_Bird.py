# -*- coding: utf-8 -*-
"""
Created on Sun Apr 19 13:58:53 2020
...
final_gen_list.append(0)
pygame.font.init()  # init font
pygame.mixer.init(frequency=14050, size=-16, channels=2, buffer=4096, devicename='None', allowedchanges=AUDIO_ALLOW_FREQUENCY_CHANGE | AUDIO_ALLOW_CHANNELS_CHANGE)
#BEGIN_SOUND = {}
BEGIN_SOUND = pygame.mixer.Sound('Flappy.wav')
END_SOUND = pygame.mixer.Sound('GameOver.wav')
WIN_WIDTH = 600
WIN_HEIGHT = 800
FLOOR = 730
...
def eval_genomes(genomes, config):
   ...
   # score_list.append(0)
    ...
    '''plotting graph'''
        #plt.scatter(score,gen,label = 'real',color ='blue')
    ...
    while run and len(birds) > 0:
        ...
        for x, bird in enumerate(birds):  # give each bird a fitness of 0.1 for each frame it stays alive
            ...
            BEGIN_SOUND.set_volume(0.4)   
            BEGIN_SOUND.set_volume(0.3)
            BEGIN_SOUND.set_volume(0.2)
            BEGIN_SOUND.set_volume(0.1)
        ...
        if len(birds) == 0:
            ...
            final_gen_list.append(gen)
            print(score,gen)
            plt.figure(figsize=(10,10)) 
            plt.plot(gen_list,score_list, color ='red',scalex = True,scaley = True )
...
def run(config_file):
    ...
    plt.figure(figsize =(10,10))
    plt.plot(final_gen_list,final_score_list,label='score' ,color ='red')
    plt.xlabel('GENERATION')
    plt.ylabel('SCORE')
    plt.legend()
    plt.title("GENERATION vs SCORE ")
